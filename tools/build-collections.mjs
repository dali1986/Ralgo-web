...
export async function buildCollections(root) {
  ...
  for(const [id,c] of collections){
    ...
    let html=shell({title:m.title,description:m.description,path,image:image(cover)||shareImage,imageAlt:cover?.title||m.title,schema:{'@type':'CollectionPage',name:m.title,description:m.description,url:absolute(path),creator:creator(id),mainEntity:{'@type':'ItemList',numberOfItems:c.items.length,itemListElement:c.items.map((w,i)=>({'@type':'ListItem',position:i+1,name:w.title,url:absolute(workPath(id,w.number))}))}}});
    ...
    for(let i=0;i<c.items.length;i++){
      ...
      let workHTML=shell({title:w.title,description,path:workURL,image:image(baseWork)||shareImage,imageAlt:image(baseWork)?baseWork.title:'Ralgo exhibition',schema:artSchema});
